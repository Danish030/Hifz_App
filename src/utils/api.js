...
export async function fetchBookmarks() {
  const { data } = await axios.get('/api/user/bookmarks', { withCredentials: true });
  return data?.bookmarks || data?.data || data || [];
}

/** Fetch user collections */
export async function fetchCollections() {
  const { data } = await axios.get('/api/user/collections', { withCredentials: true });
